...
class WeatherForecastDataset(Dataset):
    def __init__(self, data, input_seq_len, target_seq_len):
        self.data = data
        self.input_seq_len = input_seq_len
        self.target_seq_len = target_seq_len

    def __len__(self):
        return len(self.data) - self.input_seq_len - self.target_seq_len + 1

    def __getitem__(self, idx):
        input_seq = self.data[idx : idx + self.input_seq_len]
        target_seq = self.data[
            idx + self.input_seq_len : idx + self.input_seq_len + self.target_seq_len
        ]
        return torch.tensor(input_seq, dtype=torch.float32), torch.tensor(
            target_seq, dtype=torch.float32
        )


input_seq_len = 24
target_seq_len = 8
dataset = WeatherForecastDataset(normalized_data, input_seq_len, target_seq_len)
dataloader = DataLoader(dataset, batch_size=32, shuffle=True)

# Build your model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
src_vocab_size = normalized_data.shape[1]
